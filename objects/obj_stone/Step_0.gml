...
part_type_speed(particle_trail, 0.1, 0.3, 0, 0);
//end
...
if(!dead && (cooltime <= 0))
{	
	instance_create_layer(x, y, "Bullet_Layer", obj_bullet_stone);
...
{	
	...
	move_towards_point(current_x, current_y, spd / 30 + dis / 300);
	
	if(cooldown <= 0 || point_distance(x, y, current_x, current_y) < 30) 
	{
		cooldown = random_range(1, global.spd * 5);
