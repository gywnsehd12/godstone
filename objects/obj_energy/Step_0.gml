...
part_type_speed(particle_trail, 0.3, 0.5, 0, 0);
//end
...
if(!dead && (cooltime <= 0))
{	
	instance_create_layer(x, y, "Bullet_Layer", obj_bullet_energy);
...
{	
	...
	if(cooldown <= 0 || point_distance(x, y, current_x, current_y) < 100) 
	{
		cooldown = random_range(0, global.spd * 1);
		
		current_x = obj_player.x + random_range(-800, 800);
		current_y = obj_player.y + random_range(-800, 800);
...
if(!dead && hp <= 0) 
{
	...
	image_alpha = 1;
	obj_controller.game_score += 5;
}

if(dead)
{
	part_type_speed(particle_death, 1, 3, 0, 0);
